...
```{r, tableest, echo = FALSE, results = "asis"}
#tab <- read.csv("c:/tmp/paper BFpack/table1.csv")
tab <- read.csv("/Users/jorismulder/surfdrive/paper BFpack/BFpack paper Github/BFpack_paper/table1.csv")
tab <- as.matrix(tab)
papaja::apa_table(tab, note = "R functions, packages, descriptions of tests, parameter of interest, and example name of the parameter that is tested, and the classification whether it is an exact or approximate Bayes factor test. For this table we assume that 'y1' is the label of an outcome variable, 'x1' is the label of a numeric predictor variable, and 'g1' is the label of a level of a grouping (factor) variable. The actual names that are used depend on the names of the variables in the model.", escape =TRUE)
```

 ```{r, tablehyp, results = "asis"}
 papaja::apa_table(read.csv("/Users/jorismulder/surfdrive/paper BFpack/BFpack paper Github/BFpack_paper/table2.csv"), caption =  "Example hypothesis tests that can be executed using `BFpack`.")
 ```
...
\begin{equation}
H_t:\textbf{R}^E\boldsymbol\theta=\textbf{r}^E~\&~\textbf{R}^O\boldsymbol\theta>\textbf{r}^O,
\label{Ht1}
\end{equation}
...
\begin{equation}
\pi_t(\boldsymbol\theta^O,\boldsymbol\phi) = \pi_u(\boldsymbol\theta^O,\boldsymbol\phi|\boldsymbol\theta^E=\textbf{r}^E) \times \pi_u(\boldsymbol\theta^E=\textbf{r}^E)^{-1}\times P(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E)^{-1}\times I(\boldsymbol\theta^O>\textbf{r}^O),
\label{truncprior}
\end{equation}
...
\begin{equation}
B_{tu}^F = \frac{\pi_u(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y})}{\pi_u^*(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y}^b)}\times \frac{P_u(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y})}{P_u^*(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y}^b)},
\label{FBFtu}
\end{equation}
...
\begin{equation}
B_{tu}^F = \frac{\mathcal{T}(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y})}{\mathcal{C}(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y}^{\textbf{b}})}\times \frac{\mathcal{T}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y})}{\mathcal{C}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y}^{\textbf{b}})},
\label{FBFtuMVN}
\end{equation}
...
First the nuisance parameters are integrated out to yield the marginal posterior for $(\boldsymbol\theta^E,\boldsymbol\theta^O)$. Following large sample theory [Ch. 4 @Gelman2014], this posterior can then be approximated with a multivariate Gaussian distribution using the MLE and error covariance matrix. The approximated Gaussian posteriors can then be used to get estimates of the posterior quantities in the numerators in Equation \ref{FBFtu}. The corresponding default prior for $(\boldsymbol\theta^E,\boldsymbol\theta^O)$ is obtained by raising the posterior to a minimal fraction $b$, which results in a multivariate Gaussian distribution where the error covariance matrix is multiplied with the reciprocal of the minimal fraction, and the mean is shifted towards the boundary of the constrained space. This default Bayes factor can then be written as
\begin{equation}
B^F_{tu} = \frac{\mathcal{N}(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y})}{\mathcal{N}(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y}^b)}\times \frac{\mathcal{N}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y})}{\mathcal{N}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y}^b)},
\label{FBFtuBain}
\end{equation}
...
Hence, in order to compute the approximated Bayes factor in Equation \ref{FBFtuBain} only the MLEs, the error covariance matrix, and the sample size are needed. The minimal fraction is then set equal to the number of constraints that are tested divided by the sample size [@Gu2019]. These three elements can simply be extracted from fitted model objects obtained using other packages in `R`. Currently `BFpack` supports objects of class `glm`, `coxph`, `polr`, `survreg`, and `zeroinfl`. When executing `BF()` on an object of these classes, the function `BF.default` is called which extracts the estimates, the error covariance matrix, and the sample size from the fitted model object to compute Equation \ref{FBFtuBain} for the hypotheses of interest. Thus, this Bayes factor can easily be used for testing hypotheses under other model classes by applying the function `BF()` to a named vector with the estimates, its corresponding error covariance matrix, and the sample size.
...
\begin{equation}
B^F_{tu} = B^F_{t'u} \times \frac{\mathcal{IG}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y})}{\mathcal{IG}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y}^b)},
\label{FBFtuVar}
\end{equation}
where $B^F_{t'u}$ denotes the fractional Bayes factor of hypothesis $H_{t'}:\boldsymbol\theta^E=\textbf{r}^E$ [i.e., hypothesis $H_t$ where the order constraints are omitted, see also @Pericchi2008] against $H_u$, and $\mathcal{IG}$ denotes an unconstrained inverse gamma distribution. We refer the interested reader to [@BoingMessing2017] for the mathematical expressions and derivations.
...
\begin{equation}
B_{tu} = B_{t'u} \times \frac{\text{shifted-}\mathcal{F}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E,\textbf{Y})}{\text{shifted-}\mathcal{F}(\boldsymbol\theta^O>\textbf{r}^O|\boldsymbol\theta^E=\textbf{r}^E)},
\label{BFtuICC}
\end{equation}
...
\begin{eqnarray*}
\pi_u(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y}^o) &=& \int \pi_u(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y}^o,\textbf{Y}^m) \pi_u(\textbf{Y}^m|\textbf{Y}^o) d\textbf{Y}^m\\
&\approx& M^{-1}\sum_{m=1}^M \pi_u(\boldsymbol\theta^E=\textbf{r}^E|\textbf{Y}^o,\textbf{Y}^{(m)}),
\end{eqnarray*}
...
In the case the class of the fitted model `x` is not supported (Table 1), `BF.default()` is called which executes the approximate Bayes factor. In this case, the following (additional) arguments are required:
\begin{itemize}
\item `x`, a named numeric vector of the estimates (e.g., MLE) of the parameters of interest where the names are equal to the names of the parameters.
\item `sigma`, the approximate posterior covariance matrix (e.g,. error covariance matrix) of the parameters of interest.
\item `n`, the sample size that was used to acquire the estimates and covariance matrix.
\end{itemize}
...
In the 8th line, `t_test` function renders classical right one-sided $t$ test and stores the result in object `ttest1`, which contains $t$ value, degree of freedom, and $p$ value, as well as $95\%$ confidence interval: \\
...
It is interesting to see how the posterior probabilities for the hypotheses from the exploratory test relate to the p values of the classical significance tests of $H_0:\beta=0$ versus $H_1:\beta\not =0$. This can be checked by running:
```{r eval = FALSE, echo = TRUE}
ct <- lmtest::coeftest(aov1)
```
This results in p values of $0.66697$, $1.842$e$-10$, $1.410$e$-06$, $0.01115$ for `(Intercept)`, `anchorrounded`, `motivationlow`, and `anchorrounded:motivationlow`, respectively. Thus, when using a significance level of 0.05 we would conclude that there is enough evidence in the data to conclude that an interaction effect is present. The Bayesian test on the other hand suggests that we have to be more cautious as there is still a posterior probability of 0.144 that no interaction effect is present. This illustrates that Bayes factor tests are generally more conservative than classical significance tests, which is one of the reasons for the recent recommendations to use smaller significance levels than the traditional choice of 0.05 [@Benjamin2017].
...
Even though listwise deletion is generally not recommended [@Rubin1987; @Rubin1996], for this illustration we compute the Bayes factors and posterior probabilities based on these 6 complete data observations to illustrate the loss of evidence as a result of list-wise deletion.
```{r, echo = TRUE, eval = FALSE}
...
Previous research had shown that the facial width-to-height ratio (fWHR) has a postive effect on perceived aggression and thus may also have a positive effect on sentencing outcomes. In addition, perceived Afrocentricity had been shown to be associated with harsher sentences [@Wilson2015]. In the first hypothesis it was expected that all three predictors have a positive effect on the probability of being sentenced to death. Additionally, we might expect lack of perceived trustworthiness to have the largest effect. In the second hypothesis it was assumed that only trustworthiness has a positive effect. Finally, the complement hypothesis was considered. The hypotheses can then be summarized as follows
\begin{eqnarray*}
...
Finally we illustrate how an exploratory Bayes factor test can be executed from output of a classical significance test:
```{r, echo = FALSE, eval = FALSE}
...
the Bayes Factor for $H_{1}$ against $H_{A}$ was approximately $4631.01$ and the posterior probability for $H_{1}$ was effectively 1. Thus the order-constrained analysis indicate decisive support for the researchers' hypothesis. 
